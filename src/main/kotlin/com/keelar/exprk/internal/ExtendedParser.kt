...
internal class ExtendedParser(private val tokens: List<Token>) : Parser {
	...
	override fun parse(): Expr {
		...
		if (!isAtEnd()) throw ExpressionException("Expected end of expression, found '${peek().lexeme}'")
	...
	private fun assignment(): Expr {
		...
		if (match(ASSIGN)) {
			...
			} else throw ExpressionException("Invalid assignment target")
		}

		return expr
	}

	private fun oror(): Expr {
		var expr = andand()
	...
	private fun call(): Expr {
		if (matchTwo(IDENTIFIER, LEFT_PAREN)) {
			...
			if (!check(RIGHT_PAREN))
				do arguments += expression() while (match(COMMA))
	...
	private fun matchTwo(first: TokenType, second: TokenType): Boolean {
		...
		if (match(first) && match(second)) return true
